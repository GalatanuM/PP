...
class Polyglot {
    ...
    private static int SumCRC(String token){
        ...
        Value result = polyglot.eval("python", "sum(ord(ch)*ord(ch)*ord(ch)*ord(ch)*ord(ch)%256 for ch in '" + token.substring(1,token.length()-1) + "')");
    ...
    public static void main(String[] args) {
        int sumaControl;
        System.out.print("Introduceti suma de control:");
        Scanner input = new Scanner(System.in);
        sumaControl=input.nextInt();
